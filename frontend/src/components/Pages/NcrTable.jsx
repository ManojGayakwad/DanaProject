...
const NcrTable = () => {
  ...
  const [openNcr, setOpenNCR] = React.useState(false);
  ...
  const { loading2, data2 } = useSelector((state) => state.roleReducer);
  const { loading3, data3 } = useSelector((state) => state.roleReducer);
  const { loading4, data4 } = useSelector((state) => state.NcrOptionReducer);
                  ...
                  <div>
                    ...
                    <Box sx={{ minWidth: 350, mt: 3 }}>
                      <FormControl fullWidth>
                        ...
                        >
                          <option>Select</option>
                          {data4[0].ncr.map((item) => (
                            <option value={item.ProcessStage}>
                              {item.ProcessStage}
                            </option>
                          ))}
                        </NativeSelect>
                    ...
                    <Box sx={{ minWidth: 350, mt: 3 }}>
                      <TextField
                        fullWidth
                        id="outlined-basic"
                        label="Rework Man-Hrs"
                        variant="filled"
                        type="number"
                        required
                        name="ReworkHrs"
                        onChange={(e) => setReworkHrs(e.target.value)}
                        defaultValue={ReworkHrs}
                      />
                    </Box>
                    <p>{Error.ReworkHrs}</p>
                    <Box
            ...
            {data[0].ncr.map((item, index) => (
              <tr key={index}>
                <React.Fragment key={item.Id}>
                  <td
                    style={{ width: "20px", color: "blue", cursor: "pointer" }}
                    onClick={(e) => {
                      getncriddetails(
                        item._id,
                        item.Id,
                ...
                {localStorage.getItem("isAdmin") === "true" ? (
                  <div style={{ display: "flex" }}>
                    ...
                    >
                      <Edit fontSize="inherit" sx={{ color: "#4cbb17" }} />
                    </IconButton>
                    <IconButton
                      aria-label="delete"
                      size="large"
                    ...
                    >
                      <DoneAllIcon color="success" variant="contained">
                        Submit
                      </DoneAllIcon>
                    </IconButton>
                  </div>
                ) : localStorage.getItem("username") === item.CreatorId ? (
                  <td>
                    <div style={{ display: "flex" }}>
                      <IconButton
                        aria-label="edit"
                  ...
                  <td style={{ width: "20px" }}>NA</td>
                )}
              </tr>
            ))}
          </tbody>
        </table>
        <Dialog open={openAL} onClose={handleclosefinalAlert}>
